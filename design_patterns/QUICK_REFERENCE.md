...
**Solution**: Private constructor, static getter
...
```cpp
unique_ptr<Shape> factory->createShape("circle");
```
...
```cpp
House h = HouseBuilder().windows("glass").doors("wooden").build();
```
...
```cpp
class VLCAdapter : public MediaPlayer { /* adapts VLCPlayer */ };
```
...
```cpp
auto coffee = make_unique<MilkDecorator>(make_unique<SimpleCoffee>());
```
...
```cpp
class ProxyImage : public Image { /* controls RealImage */ };
```
...
**Solution**: Encapsulate algorithms, make interchangeable
...
```cpp
invoker->executeCommand(make_unique<TurnOnCommand>());
```
**When**: Parameterize actions, undo/redo, queuing
**Avoid**: Simple calls (function pointers sufficient)
...
```cpp
void process() { read(); validate(); save(); } // template
```
...
- ✅ `make_unique`/`make_shared` (creation)
- ✅ `auto` (type deduction)
- ✅ `override` (virtual safety)
- ✅ `= default`/`= delete` (explicit defaults)
- ✅ `nullptr` (null safety)
...
g++ -std=c++11 -o pattern pattern.cpp
