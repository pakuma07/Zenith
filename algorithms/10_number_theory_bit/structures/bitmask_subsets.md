...
The idiom `for (sub = mask; sub; sub = (sub - 1) & mask)` walks those submasks
...
vector<int> allSubsets(int n) {
    ...
    for (int mask = 0; mask < total; ++mask) {
        // To read members: for (int i = 0; i < n; ++i) if (mask & (1 << i)) ...
...
vector<int> subMasks(int mask) {
    ...
    for (int sub = mask; sub; sub = (sub - 1) & mask)
        subs.push_back(sub);
    subs.push_back(0);                   // the loop stops before the empty submask
    return subs;
}
```
...
```cpp
vector<int> s = allSubsets(3);   // 8 masks: 0,1,2,3,4,5,6,7
vector<int> m = subMasks(0b101); // submasks of {0,2}: 0b101, 0b100, 0b001, 0b000
(void)s; (void)m;
```
