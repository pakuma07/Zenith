...
// large results; use long long and keep inputs small.
long long power(long long base, long long exp) {
    long long result = 1;
    while (exp > 0) {
        if (exp & 1LL)          // current low bit set -> multiply base in
            result *= base;
        base *= base;           // square the base for the next bit
        exp >>= 1LL;            // move to the next exponent bit
    }
    return result;
}

// Modular exponentiation: base^exp mod m. Reduces after every multiply so
// intermediate values stay below m^2 and never overflow long long for m < ~3e9.
long long modPow(long long base, long long exp, long long m) {
    long long result = 1 % m;   // handles m == 1 (everything is 0 mod 1)
    base %= m;
    while (exp > 0) {
        if (exp & 1LL)
...
```cpp
long long p = power(3, 10);                  // 59049
long long m = modPow(2, 10, 1000000007LL);   // 1024
(void)p; (void)m;
```
