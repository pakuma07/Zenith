...
struct BinaryLifting {
    ...
    BinaryLifting(const vector<vector<int> >& adj, int root) {
        int n = (int)adj.size();
        LOG = 1;
        while ((1 << LOG) < n) ++LOG;      // smallest LOG with 2^LOG >= n
        depth.assign(n, 0);
        up.assign(LOG + 1, vector<int>(n, -1));

        // DFS (iterative) to fill depth and direct parents up[0]
        vector<pair<int,int> > stk;        // (node, parent)
        stk.push_back(make_pair(root, -1));
        while (!stk.empty()) {
            int v = stk.back().first, p = stk.back().second;
            stk.pop_back();
            up[0][v] = p;
    ...
    int kthAncestor(int v, int dist) const {
        for (int k = 0; k <= LOG && v != -1; ++k)
            if (dist & (1 << k)) v = up[k][v];
        return v;
    }
