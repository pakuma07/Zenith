...
int tsp(const vector<vector<int> >& cost) {
    ...
    for (int mask = 1; mask < full; ++mask) {
        for (int i = 0; i < n; ++i) {
            ...
            if (!(mask & (1 << i))) continue;  // i must be inside mask
            for (int j = 0; j < n; ++j) {
                if (mask & (1 << j)) continue; // skip already-visited city
                int next = mask | (1 << j);
    ...
    int best = INF;
    for (int i = 0; i < n; ++i)
        best = min(best, dp[full - 1][i] + cost[i][0]);
    return best;
}
```
...
int minTour = tsp(cost);   // shortest cycle 0 -> ... -> 0, returns 80
(void)minTour;
```
