...
// Returns indices (i, j) with a[i] + a[j] == target on a SORTED ascending array,
...
```cpp
vector<int> a = {1, 3, 4, 5, 7, 11}; // must be sorted ascending
pair<int,int> p = twoSumSorted(a, 9); // a[1]+a[4] = 3+7 = 9 -> (1, 4)
(void)p;
```
