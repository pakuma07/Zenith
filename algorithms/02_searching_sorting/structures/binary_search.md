...
// Returns an index of target in the SORTED vector a, or -1 if absent.
int binarySearch(const vector<int>& a, int target) {
    int l = 0, r = (int)a.size() - 1;   // inclusive search window [l, r]
    while (l <= r) {
...
```cpp
vector<int> a = {1, 3, 5, 7, 9};   // must be sorted
int idx = binarySearch(a, 7);      // idx == 3
(void)idx;
```
