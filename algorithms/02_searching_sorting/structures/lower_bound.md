...
// Returns the index of the first element >= target in the SORTED vector a.
// If every element is < target, returns a.size() (the insertion point).
int lowerBound(const vector<int>& a, int target) {
    int lo = 0, hi = (int)a.size();      // half-open range [lo, hi)
    while (lo < hi) {
        int mid = lo + (hi - lo) / 2;
        if (a[mid] < target)
...
```cpp
vector<int> a = {1, 3, 3, 5, 7};    // must be sorted
int i = lowerBound(a, 3);           // i == 1 (first element >= 3)
int j = lowerBound(a, 4);           // j == 3 (insertion point for 4)
(void)i; (void)j;
```
