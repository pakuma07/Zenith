...
long long maxSubarray(const vector<int>& a) {
    long long best = a[0];   // global best so far
    long long cur  = a[0];   // best subarray sum ENDING at the current index
    for (size_t i = 1; i < a.size(); ++i) {
        // Either start fresh at a[i], or extend the previous run.
        cur = max((long long)a[i], cur + a[i]);
        best = max(best, cur);              // update global maximum
    }
    return best;
}
```
...
long long ans = maxSubarray(a); // subarray {4,-1,2,1} sums to 6
(void)ans;
```
