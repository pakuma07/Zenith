...
int minArrows(std::vector<std::pair<int,int>> intervals) {
    ...
    int arrows = 1;
    int shot = intervals[0].second;            // first arrow at first end
    for (size_t i = 1; i < intervals.size(); ++i) {
        if (intervals[i].first > shot) {        // current interval not covered
            ++arrows;                           // need a new arrow
            shot = intervals[i].second;
        }
    }
    return arrows;
}
...
int main() {
    std::vector<std::pair<int,int>> balloons = {{10,16},{2,8},{1,6},{7,12}};
    std::cout << minArrows(balloons) << "\n";  // 2
    return 0;
}
```
