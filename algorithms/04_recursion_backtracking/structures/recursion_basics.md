...
long long factorial(int n) {
    ...
    return (long long)n * factorial(n - 1); // recursive case: smaller subproblem
}

// Sum of a vector over [i, end): another reduce-and-combine recursion.
long long sumFrom(const vector<int>& a, size_t i) {
...
```cpp
long long f = factorial(5);          // 5! = 120
vector<int> a = {4, 2, 8, 1};
long long s = sumFrom(a, 0);         // 4+2+8+1 = 15
(void)f; (void)s;
```
