...
struct SegTree {
    int n;
    vector<long long> t;   // t[1] is the root; children of v are 2v and 2v+1

    explicit SegTree(const vector<long long>& a)
        : n((int)a.size()), t(4 * a.size(), 0) {
    ...
    long long query(int v, int lo, int hi, int ql, int qr) {
