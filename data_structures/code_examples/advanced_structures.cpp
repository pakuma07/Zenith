...
class DSU {
private:
    vector<int> parent;  // parent[x] = representative parent of x (self if root)
    vector<int> rankv;   // upper bound on the height of the tree rooted at x
...
private:
    int n;
    vector<int> bit;  // 1-based tree; bit[i] sums (i - lowbit, i]
public:
    ...
    int rangeSum(int l, int r) const {
        return prefixSum(r) - (l ? prefixSum(l - 1) : 0);  // guard l==0 (no prefix below it)
    }
};
...
private:
    int n;
    vector<int> tree;  // implicit binary tree: children of node are 2*node, 2*node+1
    ...
    int queryRec(int node, int l, int r, int ql, int qr) const {
...
public:
    ...
    int query(int l, int r) const { return queryRec(1, 0, n - 1, l, r); }
};
...
int main() {
    ...
    dsu.unite(1, 2);
    dsu.unite(3, 4);
    cout << "find(0) == find(2): " << (dsu.find(0) == dsu.find(2)) << endl;
    cout << "find(0) == find(4): " << (dsu.find(0) == dsu.find(4)) << endl;

    cout << "\n== Fenwick Tree ==" << endl;
    Fenwick fen(5);
    vector<int> arr = {1, 2, 3, 4, 5};
    for (int i = 0; i < (int)arr.size(); ++i) fen.update(i, arr[i]);
    cout << "rangeSum(1,3) = " << fen.rangeSum(1, 3) << endl;

    cout << "\n== Segment Tree ==" << endl;
    SegmentTree seg(arr);
    cout << "query(0,4) = " << seg.query(0, 4) << endl;
    seg.update(2, 10);
    cout << "after update idx 2 -> 10, query(0,4) = " << seg.query(0, 4) << endl;
